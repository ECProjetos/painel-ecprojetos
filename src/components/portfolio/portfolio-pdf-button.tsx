...
function formatCommercialCurrency(
  value: number,
  currency: string,
) {
  const prefix =
  ...
  async function generatePdf() {
    ...
    if (
      ...
      metrics.push({
        ...
        value: `${project.estimated_hours.toLocaleString(
          "pt-BR",
        )} h`,
      })
    }
    ...
    const maxClassificationLines =
      Math.max(
        ...classificationLines.map(
          (lines) =>
            lines.length,
        ),
      )

    const classificationHeight =
      Math.max(
        20,
        13 +
          maxClassificationLines *
            3.8,
      )
